...
2. `AUTH=missing` → separate network from auth:
   `curl -fsS https://tirelesscode.com/api/regions` failing → E_NET_PLATFORM;
   otherwise → E_CODER_LOGIN.
...
| `E_NET_PLATFORM` | platform API unreachable | Check `curl -fsS https://tirelesscode.com/api/regions`; probe general connectivity; ask about VPN/proxy/firewall. Unresolvable → support summary. |
